...
// Ergaenzungen Hermann Bach 2025:
// possibility to change I2c-connection to non standard Wire
...
{
  int retVal = -1;
  pI2c->beginTransmission(_ams5600_Address);
  pI2c->write(in_adr);
  pI2c->endTransmission();
  pI2c->requestFrom(_ams5600_Address, (uint8_t) 1);
  while (pI2c->available() == 0)
    ;
  retVal = pI2c->read();
...
word AMS_5600::readTwoBytesTogether(int addr_in)
{
  ...
  pI2c->write(addr_in);
  pI2c->endTransmission();
  pI2c->requestFrom(_ams5600_Address, (uint8_t) 2);
  while (pI2c->available() < 2)
    ;
  
  int highByte = pI2c->read();
  int lowByte  = pI2c->read();
...
// *******************************************************
float AMS_5600::getAngleDegrees()
{
  // Raw data reports 0 - 4095 segments, which is 0.087921 of a degree
  float retVal = getRawAngle() * 360.0/4095.0;
  return retVal;
}

void AMS_5600::setWire(TwoWire *pi2c)
{
 pI2c = pi2c;
}
